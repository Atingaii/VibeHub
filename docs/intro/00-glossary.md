...
### NATS / JetStream
NATS 是轻量级消息中间件；JetStream 是 NATS 的持久化扩展（消息落盘 + ACK 确认 + 延迟投递）。VibeShop 用它跑 Feed 写扩散、订单超时、AI 总结触发。
→ 详见 [ADR-004](../adr/004-nats-messaging.md)

### Topic（主题）
消息按 topic 分类。"feed.push.fanout"是写扩散的 topic，"order.timeout"是订单超时。生产者发到 topic，消费者订阅 topic。
...
### 延迟投递
"30 分钟后再处理"。订单 30 分钟未支付要自动取消——发条延迟消息，到点了消费者才收到。
