...
>
> 选 `/health` 是因为它**真的访问了所有底层依赖**（MySQL / PG / Redis）——一次请求把整个基础设施栈都触达一遍，最适合走读。
>
> 1.1 起还有一条业务端点 `POST /api/v1/auth/register`（用户注册）已可用，业务侧请求生命周期见 [04-feature-tour.md 链路 0](04-feature-tour.md#链路-0--用户注册已实现)。本文聚焦基础设施侧。
>
