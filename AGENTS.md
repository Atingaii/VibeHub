...
**功能级文档必须含「面试官 Q&A」段**（写在文档末尾，紧邻修订脚注前）：

定位：本项目同时是简历项目，feature 文档承担"面试官交叉盘问"的预演价值。每份 `docs/features/*.md` 都要有面试官 Q&A，让读者（包括未来的你）一眼就能回答"这块为什么这么做"。

条数与风格：

- **不固定条数**——按文档信息密度走。业务核心 / 多次 review 修订过的子项一般 5–8 条；薄子项 2–4 条；DEPRECATED 文档可跳过（首行已有 deprecation 标记 + 指向新文档）。
- **trade-off 领型为主**——选了 A 不选 B 的决策、与流行做法的偏离、看似反直觉的安排。
- **混入少量 what-if 与 boundary**——换技术栈 / 加规模 / 极限场景。
- **欢迎场景题**——子项天然涉及具体业务场景时（如订单超时、分布式锁、消息幂等），用"线上发生 X，怎么定位 / 怎么扛"形式提问，比纯概念题更能体现深度。
...
> 以 trade-off 领型为主，混入 what-if / boundary / 场景题；
> 每条 Q 标注重点（**trade-off** / **what-if** / **boundary** / **scenario**）。

**Q1（trade-off）：<具体问题>？**

A: <50–200 字回答，含本项目选了什么 + 为什么 + 替代方案 + 替代不选的具体理由>。
锚点：`internal/.../foo.go:func` / 设计文档 v3 修订段。

**Q2（scenario）：线上突然 <某现象>，你怎么定位？**

A: <按本项目可观测能力 + 代码锚点拆解排查路径>。
```

挑选 Q 的标准（"会被问的"）：

- 选了 A 不选 B 的决策（cost=10 vs 12 / SHA-256 vs HMAC / Lua vs WATCH+MULTI）
- 设计中显眼的"反直觉"做法（统一 INVALID_CREDENTIALS / refresh 不走 bcrypt）
- 与流行做法的偏离（不在 access token 塞业务字段 / 不做"全设备登出"）
- 安全/并发的关键点（hash 校验 vs 仅签名 / 原子轮换为何必须）
- 与本子项强相关的线上场景题（库存超卖、消息积压、连接池打爆、Redis 内存炸）

**不要**写空泛的"为什么用 Go / 为什么用 Redis"——这种泛问题面试官也不会问，写了反而稀释信号。
